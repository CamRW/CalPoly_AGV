...
class ImageStitching(Node):
    ...
    def __init__(self):
        ...
        self.publisher_ = self.create_publisher(Image, '/video/stitched_frames', 10)
        self.subscription = self.create_subscription(Image, '/camera/color/image_raw', self.realsense_callback, 2)
        timer_period = 0.03 #seconds
        self.timer = self.create_timer(timer_period, self.stitching_callback)


        self.cap1 = cv2.VideoCapture("/dev/right_fisheye") # Fisheye Back
        self.cap2 = cv2.VideoCapture(6) # Left, video6
        self.cap3 = cv2.VideoCapture(12) # Right, video12
